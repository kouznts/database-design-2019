...
----------------------------------------------------
-- процедура выводит количество сегодн€шних рейсов
----------------------------------------------------
...
----------------------------------------------------
----------------------------------------------------
-- процедура считает количество проданных на сегодн€ билетов
----------------------------------------------------
...
begin	
	...
	select count(*) as количество_проданных_на_сегодн€_билетов
	from ticket t
	inner join flight f
...
select 
	price as цена,
	passenger_id as номер_пассажира,
	f.departure_date as дата_отбыти€
from ticket t
inner join flight f
on t.flight_id = f.flight_id
...
----------------------------------------------------
----------------------------------------------------
-- 2)	создайте хранимые процедуры дл€ добавлени€ и 
-- изменени€ записей в таблицах, 
-- провер€ющих существование редактируемой записи и 
-- определ€ющие значение суррогатного первичного ключа по умолчанию дл€ новых записей
----------------------------------------------------
...
select 
	pilot_id as номер, 
	full_name as ‘»ќ, 
	number_of_flights as количество_полЄтов,
	admission_group as группа_допуска
from pilot
order by full_name;
----------------------------------------------------
exec add_pilot 
	@full_name = 'ћихайлов —емЄн »ванович', 
...
-- 6) cоздайте функцию
----------------------------------------------------
-- a)	возвращающую скал€рное значение (тип функции Scalar)
----------------------------------------------------
...
select
	...
	takeoff_weight_ as взлЄтна€_масса,
	height_ as высота,
	speed_ as скорость
